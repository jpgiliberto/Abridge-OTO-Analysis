...
KeyUserHigh <- SessionsRaw |> 
  ...
  #8/19 median number of total usage was 106, top third was 135 will need to update
  mutate(HighUser = n>135) #|> 
  #select(-n)
...
LMWOWHData <- SignalRaw |>
  ...
  left_join(KeyUserHigh) |> 
  filter(HighUser) |> 
  mutate(HrsPerDay = WOWHmontly*20/`per Appointment`/60) |> 
...
###Descriptive how much time saved per appointment, estimated hours saved
```{r}
TimeInNotesAIonly <- SignalRaw |>
  filter(Metric == "Time in Notes per Appointment") |>
  filter(GroupData =="AIScribe") |> 
  left_join(KeyUserHigh) |> 
  left_join(BaselineNotesPerAppt, join_by (StudyID, Metric)) |> 
  #Change in Notes is the observed value subtracted from the current value. We will get multiple for each month. and then average them. 
...
AverageTimeInNotesUser <- TimeInNotesAIonly |>
  group_by(StudyID,HighUser)|> 
  summarise(MeanChangeInNotesApp = round(mean(ChangeInNotesPerApp),2),
            MeanEstimatedHrsSave = round(mean(EstimatedHrsSaved),2),
            MeanPercentSaved= mean(PercentNotesSaved)) |> 
  ungroup() |> 
  gt() |> 
  fmt_percent(MeanPercentSaved)

AverageTimeInNotesUser
```
#### Visual Graph to help with data analysis. 
```{r}
...
### Descriptive of time in Notes Saved
```{r}
AverageTimeInNotesByHighUser <- TimeInNotesAIonly |> 
  group_by(HighUser) |> 
  summarise(MeanChangeInNotesApp = round(mean(ChangeInNotesPerApp),2),
            MeanEstimatedHrsSave = round(mean(EstimatedHrsSaved),2),
            MeanPercentSaved= mean(PercentNotesSaved)) |> 
...
AverageTimeInNotesByHighUser

```
### Linear Mixed Effect Model for all users 
...
At mean of 111 per month 3 hours time in notes per month
22% reduction with 1.7/7.9
