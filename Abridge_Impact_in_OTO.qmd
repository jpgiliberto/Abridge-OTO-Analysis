...
#need to validate raw visit data looks okay? when I look at mine ID928 see that they mostly match but if going to use them needs to make sure
...
#ggsave ("PJ Time All Users by thirds.jpg", device = "jpeg",path = "C://Users//jpgil//OneDrive//Research - Primary//Epic//Abridge OTO//Abridge OTO Analysis//Figures", dpi = "retina", units = "in", height = 12 ,width = 8  )
...
PajamaDescr <- Pajama |> 
  ...
  select(-Transition) |>
  mutate(Difference = AIScribe - Control) |> 
...
### WOWH Pajama Time (reduced in high users 27%)
...
### WOWH Unscheduled Day AA 43%reduced 2 hrs 4.6
...
ApptPerMoAIScribes <- LMAllNotes |> 
  filter(GroupData !="Control") |> 
  summarize(mean = mean(Denominator))

LMNotesApp <- lmer(formula = Value ~ (GroupData) + (1|StudyID), data=LMAllNotes)

summary(LMNotesApp)
 LMNotescoef <- tidy(LMNotesApp, effects = "fixed") |> 
  gt()
 LMNotescoef |> 
          data_color(columns = p.value,
              method = "bin",
...
ApptPerMoAIScribesCR <- LMAllCR |> 
  filter(GroupData !="Control") |> 
  summarize(mean = mean(Denominator))

LMCRApp <- lmer(formula = Value ~ (GroupData) + (1|StudyID), data=LMAllCR)

summary(LMCRApp)
 LMCRcoef <- tidy(LMCRApp, effects = "fixed") |> 
  gt()
 LMCRcoef |> 
          data_color(columns = p.value,
              method = "bin",
...
ApptPerMoAIScribesOrders <- LMAllOrders |> 
  filter(GroupData !="Control") |> 
  summarize(mean = mean(Denominator))

LMOrdersApp <- lmer(formula = Value ~ (GroupData) + (1|StudyID), data=LMAllOrders)

summary(LMOrdersApp)
 LMOrderscoef <- tidy(LMOrdersApp, effects = "fixed") |> 
  gt()
 LMOrderscoef |> 
          data_color(columns = p.value,
              method = "bin",
...
ApptPerMoAIScribesIB <- LMAllIB |> 
  filter(GroupData !="Control") |> 
  summarize(mean = mean(Denominator))

LMIBApp <- lmer(formula = Value ~ (GroupData) + (1|StudyID), data=LMAllIB)

summary(LMIBApp)
 LMIBcoef <- tidy(LMIBApp, effects = "fixed") |> 
  gt()
 LMIBcoef |> 
          data_color(columns = p.value,
              method = "bin",
...
### Linear Mixed Effect Model for Type of Appointments
```{r}
