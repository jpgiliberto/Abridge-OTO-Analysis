...
ggplot(notes, aes(x = StartDate, y = Value))+
  geom_point(aes(color=GroupData), size = 1.5)+
  geom_line(color = "grey60")+
  facet_grid(StudyID~HighUser, scales = "free_y",)+
  ylab("PJ Time per Scheduled Day")+
...
```
### copied from UW medicine wide to look at time saved in notes per appointment estimating the average number of 
```{r}
#| warning: false
time_in_notes_appt <- UWMdata |> 
  filter(GoLive==T) |> 
  filter(Metric == "Time in Notes per Appointment") |>
  #(month = month(date)) |> 
  left_join(UWMBaseMeans) |> 
  mutate(TimeSaving = Value - BaseMedian) |> 
  left_join(key_appt_per_mo) |> 
  mutate(TotalTimeSaved = ApptPerMo * TimeSaving /60) |> 
  left_join(key_SuperUser)
```

```{r}
PercentAmbient <- SignalRaw |> 
